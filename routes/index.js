...
const User = require('../DB_Home/Admin');
const { Patient, Payment } = require('../DB_Home/Patient')
...
router.post('/signin', (req, res)=>{
    ...
    User.findOne({username: data.username, password: data.password }).then((found)=>{
        ...
        res.redirect('back')
    }).catch((error)=>{ res.json({ msg:' Oops! - Invalid Credentials'})})
});
...
router.post('/createAdmin', (req, res)=>{
    ...
    User.findOne({username: data.username}).then((found)=>{
        if(found){
            return res.json({ msg:'Oop! - User already exist'})
        }
        User.create(data).then((newUser)=>{
            return res.redirect('/dashboard')
        })
    }).catch((error)=>{ res.json({ msg:'Sorry - you can try again later'})})
})
